...
{
  uintptr_t faultAddress = (uintptr_t)si->si_addr;
  uintptr_t beginingAddress = (uintptr_t)sqrts;
  long elements = (faultAddress - beginingAddress);
  long offsetSize = faultAddress % page_size;

  //printf("Fault mod/page_size: %li\n", offsetSize);

  //printf("%p\n",sqrts); //Beginning address of table
  //printf("%p\n",si->si_addr); //Address of fault
  
  if(munmap(sqrts, page_size) == -1){
    printf("munmap() failed %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }

  sqrts = mmap(NULL, page_size, PROT_READ || PROT_WRITE, MAP_SHARED || MAP_ANONYMOUS, -1, 0);
  ...
  calculate_sqrts(&sqrts[offsetSize/8], elements/8, 2);
  //printf("sqrts[%ld]: %f\n", offsetSize/8, sqrts[offsetSize/8]);
}
